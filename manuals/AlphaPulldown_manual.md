...
truncate_pickles.py \
  ...
  --number_of_threads=8 
```
* `--src_dir=</path/to/source>`: replace `</path/to/source>` with a path to structures output directory. The same as the `--output_path` to the `run_multimer_jobs.py` script from the [Predict structures](#2-predict-structures-gpu-stage) step.
* `--dst_dir=</path/to/destination>`: replace `</path/to/destination>` with the path of the directory very to copy the truncated results.
* `--keys_to_exclude=aligned_confidence_probs,distogram,masked_msa`: a comma-separated list of keys that should be excluded from the copied pick files. Default keys are "aligned_confidence_probs,distogram,masked_msa".
* `--number_of_threads=4`: number of threads to run parallel. 
...
**1)** Convert all models into ModelCIF in separated files.

**2)** Only convert a specific single model.

**3)** Convert a specific model to ModelCIF but keep additional models in a Zip archive associated with the representative ModelCIF formatted model.
...
#### 1. Convert all models to separated ModelCIF files
...
* `--ap_output`: path to structures directory. The same as the `--output_path` to the `run_multimer_jobs.py` script from the [Predict structures](#2-predict-structures-gpu-stage) step.
* `--monomer_objects_dir`: path to the directory containing the .pkl feature files generated in the [Compute multiple sequence alignment (MSA) and template features](#1-compute-multiple-sequence-alignment-msa-and-template-features-cpu-stage) step. Same parameter as `--monomer_objects_dir` in the `run_multimer_jobs.py` script. 
